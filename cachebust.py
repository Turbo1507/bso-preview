...
Версия больше не хардкодится руками (это и было причиной плейсхолдеров вида
"title.text" — забыли бампнуть перед пушем, до 10 минут отдавался старый
i18n.js под новую разметку). Запускается автоматически git pre-commit
хуком (см. .git/hooks/pre-commit) при любом коммите, трогающем css/js/html —
руками звать больше не нужно."""
...
TARGETS = {
    'index.html': ['css/styles.css', 'css/v2.css', 'js/i18n.js',
                   ...
                   'js/units-data.js', 'js/units-plan.js', 'js/buildings-data.js', 'js/iplan-map.js'],
