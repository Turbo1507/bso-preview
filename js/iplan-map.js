...
(function () {
  ...
  function renderPopup(building, zone) {
    ...
    const units = building.units.map(n => unitsByNumber[n]).filter(Boolean);
    const rows = units.map(u => {
      const st = u.st === 'presale' ? 'early' : u.st;
      const label = STATUS_LABEL[st][lang];
      return `<li class="iplan-popup-row is-${st}"><b>${u.n}</b><span>${u.t}</span><span>${fmtPrice(u.c)}</span><span class="iplan-status is-${st}">${label}</span></li>`;
    }).join('');
