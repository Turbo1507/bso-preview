...
const COLLAGE_LAYOUTS = {
  ...
  /* Экстерьер/бассейн — оставлен СИГНАТУРНЫМ только у 3bdsky (аэро-вид сверху,
     единственный по-настоящему непохожий на остальные ракурс). У 1bdsky/2bd/
     3bd свои detail-экстерьеры убраны из коллажа — Босс: "экстерьеры лучше
     от типа к типу не дублировать" (7 табов подряд с похожим бассейном
     выглядят как повтор). 1bd/4bd не трогал — там бассейн виден ЧЕРЕЗ окно
     интерьерного кадра (гостиная/спальня), это не отдельный «вид на бассейн»
     тайл. bso-pool-4.jpg выведен из ротации целиком (Босс: "плохое фото"). */
  '1bd': {
    cols: '1fr 1fr 1fr 1fr', rows: '293px 293px',
    tiles: [
      { photoIdx: 1, col: '1 / 3', row: '1' },                              // detail1 — спальня, широкий верх-лево
      { photoIdx: 2, col: '3', row: '1 / 3' },                              // detail2 — гостиная+бассейн (интерьер), высокий центр-право
  ...
  '1bdsky': {
    ...
    tiles: [
      ...
      { src: 'assets/bso-pool-7.jpg', col: '2 / 4', row: '1' },             // спальня (пул) — широкий верх-центр, вместо detail3-экстерьера
      { src: 'assets/bso-pool-9.jpg', col: '4', row: '1' },                 // коридор (пул) — мелкий справа
  ...
  '2bd': {
    ...
    tiles: [
      ...
      { src: 'assets/bso-pool-8.jpg', col: '3', row: '1' },                 // кухня (пул) — мелкий центр, вместо detail3-экстерьера
      { photoIdx: 2, col: '4', row: '1' },                                  // detail2 — санузел, мелкий справа
      { src: 'assets/bso-pool-11.jpg', col: '3', row: '2' },                // спальня (пул) — мелкий центр
  ...
  '3bd': {
    ...
    tiles: [
      ...
      { src: 'assets/bso-pool-3.jpg', col: '4', row: '2' }                  // кухня (пул) — мелкий справа, вместо detail3-экстерьера
    ]
  },
  '3bdsky': {
    // Единственный тип, где экстерьер (аэро-вид бассейна) осознанно оставлен —
    // он один непохож на остальные "вид сбоку на бассейн". Топвью тут и у
    // 4bd раньше был мелкой одной клеткой — рядом с двумя большими span-
    // тайлами казался игрушечным ("оч маленький топвью" — Босс 04.09).
    // Теперь топвью тоже высокая клетка (col×row1/3) — портретные пропорции
    // плана вилле подходят, размер сопоставим с соседними большими тайлами.
    cols: '1fr 1fr 1fr 1fr', rows: '293px 293px',
    tiles: [
      { photoIdx: 1, col: '1 / 3', row: '1' },                              // detail1 — спальня, широкий верх-лево
      { photoIdx: 2, col: '3', row: '1 / 3' },                              // detail2 — бассейн сверху (сигнатурный экстерьер), высокий центр
      { photoIdx: 4, col: '4', row: '1 / 3', isTopview: true },             // topview — высокий, справа
      { src: 'assets/bso-pool-2.jpg', col: '1', row: '2' },                 // спальня (пул) — мелкий слева
      { photoIdx: 3, col: '2', row: '2' }                                   // detail3 — санузел, мелкий центр
    ]
  },
  '4bd': {
    // 4 родных детальных фото + topview — пул не нужен, санузла в наборе нет.
    // Топвью — высокая клетка (см. комментарий у 3bdsky), не мелкая одна
    // клетка, как было.
    cols: '1fr 1fr 1fr 1fr', rows: '293px 293px',
    tiles: [
      { photoIdx: 1, col: '1', row: '1 / 3' },                              // detail1 — спальня, высокий слева
      { photoIdx: 4, col: '2 / 4', row: '1' },                              // detail4 — спальня+бассейн (интерьер), широкий верх-центр
      { photoIdx: 5, col: '4', row: '1 / 3', isTopview: true },             // topview — высокий, справа
      { photoIdx: 2, col: '2', row: '2' },                                  // detail2 — дуплекс/лестница, мелкий центр
      { photoIdx: 3, col: '3', row: '2' }                                   // detail3 — кабинет, мелкий центр
    ]
  }
};
